...
- **Light toggle (page 2, top-right gap — 2026-07-01):** a 74×74 square button filling the gap *under the Pagina button* and right of the ZON/BATT cards (`x:240, y:70` — matches the Pagina button's width/x, 6-px right margin). `on_short_click` → `switch.toggle` on `verlichting_vs4_switch` (`switch.verlichting_vs4_v1`, a lighting switch) — same blind-toggle pattern + `homeassistant.action` path as the gate buttons (no on/off state feedback; beeps on tap like every action button); inherits the blue button theme. It is action-only, so like the gate buttons it produces no boot-log line.
- **Tap feedback — all action buttons play a soft UI tick (2026-07-02, retuned twice same-day):** every interactive button (gate buttons Garage OPENEN/SLUITEN + Deurslot, VS4-V1, 3 AC toggles, 2 EV stop-buttons) ends its `on_short_click` with `- script.execute: play_tap`. That script plays a **short, high, soft tick** — `tap:d=4,o=7,b=240:32c` (C7 2093 Hz, ~31 ms, gain 0.025) — deliberately distinct from the over-power alarm `beep_loop` (E6 1319 Hz, 150 ms, gain 0.10). The **Pagina** page-flip button is the sole exception (silent). **Volume mechanism:** `rtttl` has *no per-play gain action* (verified in the venv `components/rtttl` — `play` takes only the song string; `gain:` is one global component setting, id `buzzer`), so `play_tap` calls the component's public C++ `set_gain()` from a lambda — drops to 0.025 for the tick, `delay: 120ms`, restores 0.10 so the alarm/doorbell keep full volume. C7 is the highest valid RTTTL note (`MAX_OCTAVE=7`). Tunable in the `play_tap` script: gain, note, BPM.
- **AC mode toggle (page 4 / devices_page — 2026-07-02):** each of the 3 AC status readouts (Woonkamer/Overloop/Slaapkamer) is now tappable — a transparent overlay `button` sits over each `ac_*_label` (108×36, card-relative x:200, y:32/70/108; clear of the top-right Pagina button, which ends at abs y:68 vs the rows at abs y≈108+). `bg_opa: TRANSP` + `border_width: 0` so the label shows through unchanged (only the tap area + LVGL press-feedback are added); the label keeps its `id`, so the existing `text_sensor` `on_value` update is untouched. **Status labels are at card-relative `x: 200`** (aligned under the button) — not the original `x: 230`: the wide **W** in `WARM` (and `DROOG`/`AUTO`) overflowed the 78 px from x:230 to the card's right edge (308) and triggered the AIRCO card's `obj` scrollbar (latent until the toggle first set heat mode — the ACs had always been cooling). x:200 gives ~108 px and lands the text exactly under the tap button. `on_short_click` **cycles cool → heat → off → cool** via `climate.set_hvac_mode`. **Secret-safe branching:** `entity_id` must stay a static `!secret` (it can't be resolved inside a C++ `!lambda` — secrets are YAML-time, lambdas are verbatim C++), so the next mode is chosen by an `if`/`else if`/`else` chain on `id(climate_*).state` rather than a templated `data:` map. Any non-cool/non-heat mode (auto/vent/dry/off) → cool. The readout reflects the *confirmed* mode once HA updates the climate (no optimistic flip, to avoid showing a mode the AC rejected). The 3 ACs are Midea units (`climate.midea_ac_*`, confirmed in the boot log). No new secrets (climate keys reused) and no new fonts → flash 72.4→72.6 %.
